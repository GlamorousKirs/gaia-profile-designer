...
// Isolated, memoized row item to stop typing keystrokes from lagging the whole panel
const SnippetRow = memo(({ 
    ...
    // Memoize text preview mutation so regex isn't re-run endlessly
    ...
    return (
        <div
            ...
            className={`group relative flex flex-col justify-between items-stretch rounded-md border p-2 text-xs font-mono cursor-pointer transition-all [contain-intrinsic-size:52px] ${
                snippet.isDefault 
...
export function SnippetPanel({ dragControls, currentCode, onSelectSnippet }: SnippetPanelProps) {
    ...
    const openEditorModal = (snippet?: Snippet, e?: React.MouseEvent) => {
        ...
        if (snippet) {
            ...
            setModalTitle("New Component Styles")
            setModalCode(currentCode.trim() || "/* CSS Properties */\n.className {\n    display: flex;\n}")
        }
        setIsModalOpen(true)
    }
        ...
        <div className="flex shrink-0 h-full relative">
            ...
            >
                <div className="h-full flex flex-col select-text overflow-hidden">
                    ...
                    <ScrollArea className="flex-1 bg-muted/20 [content-visibility:auto]">
            ...
            {/* Micro CSS Editor overlay Modal */}
            <AnimatePresence>
                {isModalOpen && (
                    <motion.div 
                        initial={{ opacity: 0 }}
                    ...
                    >
                        <div className="flex flex-col gap-2 flex-1 overflow-hidden">
                            ...
                            <textarea
                                ...
                                placeholder="/* Type CSS styles here... */"
                                spellCheck={false}
                              />
                        </div>
