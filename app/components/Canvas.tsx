...
export const Canvas = memo(function Canvas({
	...
	const integratedHtml = useMemo(() => {
		...
		let globalHeaderHtml = panelHtmlModules["/app/presets/panels_html/header.html"] || ""
		...
		if (hasPanels) {
			(["column1", "column2", "column3"] as const).forEach((colKey, index) => {
				...
				const compiled = (layoutToUse[colKey] || [])
					.filter((id: string) => id !== "columns" && id !== "header")
					.map((id: string) => panelHtmlModules[`/app/presets/panels_html/${id}.html`] || "")
					.join("\n")
