...
export const PRESETS: Preset[] = Object.entries(modules)
  .map(([path, content]) => {
    try {
      ...
      // Smart check if this specific preset configuration is inside a nested /v2/ folder
      ...
      // Generates fallback local file paths
      let cssPath = `/presets/${category}/${id}/preset.css`
      if (isVersioned) {
        cssPath = `/presets/${category}/${id}/v2/preset.css`
      }
